...
<Div custom-style = "Table or chart caption">**Percentage of last six months of life spent at home or in a community setting; `r min(basefile$fy)` - `r max(basefile$fy)`^1^** </div>
...
<Div custom-style = "Bullet 1">
Over the past `r as.character(as.english(year(end_date) - year(start_date)))` years, the percentage of time spent at home or in a community setting in the last six months of life has gradually increased from `r summarise_data(basefile, include_years = "first") %>% pull(qom)`% in financial year `r min(basefile$fy)` to `r latest$qom`% in `r latest$fy`. However, the COVID-19 pandemic and measures put in place to respond to the pandemic are likely to have contributed to the length of hospital stays which shows a marked increase in the 
percentage of time spent at home or in a community setting from 2020/21 .
</div>
