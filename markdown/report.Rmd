...
Please note that`r if_else(pub_type == "provisional", " hospital data and", "")` National Records of Scotland deaths data are provisional for `r latest$fy`. `r if_else(pub_type == "provisional", glue("This publication will be updated in {next_pub_date} with final figures. Figures for the latest year should therefore be treated with caution as there could be an undercount of hospital admissions, particularly for people who died in the last three months of the year.  This is not expected to have a large impact on these published figures when they are revised in subsequent updates."), "")`  See [Appendix 2](#appendix-2---background-information) for more information.
...
For the `r latest$deaths` residents of Scotland who died in financial year `r latest$fy`, `r latest$qom`% of their last six months of life was spent either at home or in the community (Table 1) and `r latest$qom_hosp`% of their last six months of life was spent in hospital. This is equivalent to each individual spending an average of `r if(latest$hosp < 10){as.character(as.english(latest$hosp))}else{latest$hosp}` days in hospital in the six months prior to their death (Table 1 of the [data tables](`r pub_date_link`)).
...
The NHS Board where residents spent the highest percentage of time in their last six months of life at home or in the community was `r max_hb$hb` where `r max_hb$qom`% of an individual’s last six months of life was spent at home or in a community setting and `r max_hb$qom_hosp`% in hospital. On average, this equates to approximately `r if(max_hb$hosp < 10){as.character(as.english(max_hb$hosp))}else{max_hb$hosp}` days spent in a hospital setting within the last six months of life. 
...
![Bar chart showing the percentage of end of life spent at home or in a community setting in `r latest$fy` by age group and gender. There are differences seen between the age groups and genders which are described in the text above.](figures/figure-3.png)</div>
...
Figure 4 shows that there is little variation across the different categories of deprivation. Deprivation ranges from `r min(simd$qom)`% (deprivation category `r min_simd`) to `r max(simd$qom)`% (deprivation category `r max_simd`). The trend over time by deprivation category is presented in graphical format in Figure A1.3 (see [Appendix 1](#appendix-1---trend-data)) and the corresponding data can be found in Table 4 of the [data tables](`r pub_date_link`). All deprivation categories show a similar trend over time, with a slight increase in the percentage of time in the last six months spent at home or in the community.
...
Further information and data for this publication are available from the [publication page](https://publichealthscotland.scot/) on our website.  
...
Please [provide feedback](mailto:nss.peolc@nhs.net) on this publication to help us improve our services.
...
![Series of line charts showing the percentage of end of life spent at home or in a community setting for each of the Health Boards over the financial years `r min(basefile$fy)` to `r latest$fy`. Many of the boards show a broadly consistent pattern over this ten year period.](figures/figure-a1-1.png)</div>
...
`r if_else(pub_type == "provisional", "Hospital data and ", "")`National Records of Scotland deaths data are provisional for `r latest$fy`. `r if_else(pub_type == "provisional", "Figures for the latest year should therefore be treated with caution as there could be an undercount of hospital admissions, particularly for people who died in the last three months of the year.", "")`

Estimated completeness figures for Acute Inpatient and Day Case records (SMR01) are presented in the following table. `r comp_appendix`
...
metadata %>%
  ...
  compose(i = 14, j = 2,
          value = as_paragraph(
            "Policy concepts: ",
            hyperlink_text(
              x   = paste0("http://www.gov.scot/Topics/Health/Policy/Quality-",
                           "Strategy/Quality-Measurement-Framework"),
              url = paste0("http://www.gov.scot/Topics/Health/Policy/Quality-",
                           "Strategy/Quality-Measurement-Framework"),
              props = fp_text(color = "#6c2c91",
                                           underlined = TRUE,
                                           font.size = 11)),
  ...
  compose(i = 18, j = 2,
          value = as_paragraph(
            ...
            "publication are not comparable to figures published prior to ",
            "2020. Trend information from ", min(basefile$fy), " to ", 
