...
# comp_table <- completeness(end_date)
comp_table <- read_rds(here("data", "extracts", glue("{pub_date}_completeness.rds")))

# This recodes completeness to figures as at May 2019 publication
# to allow development of relevant text referring to boards with low
# completeness. Remove this before running real publication.
comp_table %<>% 
  mutate(All = c(0.95, 0.93, 0.95, 0.88, 0.73, 0.90, 0.76,
                 0.98, 0.97, 0.92, 1, 0.93, 0.89, 0.98, 0.98, 0.87),
         Q4  = c(0.83, 0.71, 0.66, 0.54, 0.03, 0.68, 0.1, 
                 0.71, 0.87, 0.68, 0.84, 0.74, 0.71, 0.96, 0.78, 0.51))
...
data_tables <-
  glue("http://www.isdscotland.org/Health-Topics/Health-and-Social-Community-Care/Publications/",
       "{pub_date}/{pub_date}_qom.xlsm")

figures <-
    glue("http://www.isdscotland.org/Health-Topics/Health-and-Social-Community-Care/Publications/",
       "{pub_date}/{pub_date}_figures.xlsm")
...
paste("SMR01 hospital completeness for the financial year", latest$fy, "is estimated to be", comp_scot, "complete and this could have an impact on the time spent in hospital in the last six months of life.", comp_metadata, "See [Appendix 2](#appendix-2---background-information) for more information and completeness figures for each quarter. Further updates  will be made available at the following link:  [http://www.isdscotland.org/Products-and-Services/Data-Support-and-Monitoring/SMR-Completeness/](http://www.isdscotland.org/Products-and-Services/Data-Support-and-Monitoring/SMR-Completeness/)"),
paste("Due to a change in methodology, figures presented in this publication are not comparable to figures published prior to 2020. Trend information from", min(basefile$fy), "to", latest$fy, "is available in this publication. See [Appendix 3](#appendix-3---methodology) for more information."),
...
<div custom-style = "Normal_Notes">
1. `r prov_footnote``r prov_footnote`
...
Data from this publication is available to download from the [NHSScotland Open Data platform](https://www.opendata.nhs.scot/dataset/palliative-and-end-of-life-care).
...
```{r comp-text-3, echo = FALSE, results = 'asis', eval = pub_type == "provisional"}
glue("This publication will be updated in {next_pub_date} with final figures.")
```
...
In publications released prior to 2020, Care Home activities in NHS Hospital locations were counted as time spent in the Hospital setting. For publications released in 2020 onwards, these Care home activities have been excluded from our calculations for time spent in the Hospital setting and are now counted as time spent in the Community setting. These changes only affect a small proportion of Care Homes in some NHS Boards.
...
![Alt text here](figures/figure-a3-1.png)</div>
...
metadata %>%
  ...
  compose(i = 17, j = 2,
          value = as_paragraph(
            ...
            "is estimated to be ", comp_scot, " complete and this could have ",
