...
file_name <- dir_ls(here("output"), regexp = glue("{pub_date}+.*\\.xlsm$"))
file_size <- paste0("Excel ", file_size(file_name), "b")
file_desc <- c("Figures and Background Data",
               paste0("Percentage of last six months of life spent at home or ",
                      "in the community setting by financial year for each ",
                      "Health Board, HSCP, Council Area, Age, Sex, ",
                      "Deprivation and Rurality"))
file_link <- c(figures, data_tables)

list_of_tables <-
  tibble(
    file_name = 
      glue("[**{str_remove(file_name, ",
           "paste0(here('output'), '/'))}",
           "**]({file_link}) - {file_desc}"),
    file_size
  )
...
## List of Tables
<br>
```{r kable_list-of-tables, echo = FALSE}
knitr::kable(list_of_tables, 
             col.names = c("**File name**", "**File and size**"), 
             align = c('l','l'))
```
