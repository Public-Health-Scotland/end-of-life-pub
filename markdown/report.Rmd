...
file_list <- list.files(here("data", "excel-output")) 
...
for (i in 1:length(file_list)){
  ...
  paths[i] <- here("data", "excel-output", file_list[i])  # store filename (with extension included) in vector paths 
...
ListofTables <- data.frame(file_name, file_size) %>%
  mutate(file_name = case_when(
    str_detect(file_name, "qom") ~ 
      glue("[**{file_name}**]({data_tables})", 
            " - Percentage of last six months of life spent at home or ",
            "in the community setting by financial year for each Health ",
            "Board, HSCP, Council Area, Age, Sex, Deprivation and Rurality"),
    str_detect(file_name, "figure") ~
      glue("[**{file_name}**]({figures})",
