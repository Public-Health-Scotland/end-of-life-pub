...
comp_table %<>% mutate_at(vars(starts_with("Full Year")),
                          ~ c(0.95, 0.93, .95, 0.88, 0.73, 0.90, 0.76,
                              0.98, 0.97, 0.92, 1, 0.93, 0.89, 0.98, 0.87))

low_comp <- 
  comp_table %>% 
  filter_at(vars(starts_with("Full Year")), ~ . < 0.8)

comp_text <- 
  if(nrow(low_comp) > 0){
    glue("Note that there are particular issues with completeness for ",
         "{low_comp$`NHS Board` %>% glue_collapse(sep = ', ', last = ' and ')}",
         " data for the last quarter (Quarter 4) of {latest$fy} and so data",
         " for these areas in particular should be treated with caution.")
  }
