...
deaths_flags <- deaths %>%
  mutate(
    cancer = purrr::pmap_dbl(select(., contains("underlying_cause_of_")),
                                      ~any(grepl("^C[0-8]|^C9[0-7]", c(...)),
                                           na.rm = TRUE) * 1),
    
    circ_sys_dis = purrr::pmap_dbl(select(., contains("underlying_cause_of_")),
                                   ~any(grepl("^I[0-9]", c(...)),
                                        na.rm = TRUE) * 1),
    
    ischaemic = purrr::pmap_dbl(select(., contains("underlying_cause_of_")),
                                   ~any(grepl("^I2[0-5]", c(...)),
                                        na.rm = TRUE) * 1),
    
    stroke = purrr::pmap_dbl(select(., contains("underlying_cause_of_")),
                                   ~any(grepl("^I6[0-9]", c(...)),
                                        na.rm = TRUE) * 1),
    
    respiratory = purrr::pmap_dbl(select(., contains("underlying_cause_of_")),
                                   ~any(grepl("^J[0-9]", c(...)),
                                        na.rm = TRUE) * 1),
    
    copd = purrr::pmap_dbl(select(., contains("underlying_cause_of_")),
                                   ~any(grepl("^J4[0-4]", c(...)),
                                        na.rm = TRUE) * 1),
    
    dementia = purrr::pmap_dbl(select(., contains("underlying_cause_of_")),
                                   ~any(grepl("^F0[1-3]|^G30", c(...)),
                                        na.rm = TRUE) * 1),
    
    #Accidental/Falls label when creating output files
    accidental = purrr::pmap_dbl(select(., contains("underlying_cause_of_")),
                                   ~any(grepl("^W01|^W0[3-8]|^W10|^W1[7-9]", c(...)),
                                        na.rm = TRUE) * 1),
    
    covid = purrr::pmap_dbl(select(., contains("underlying_cause_of_")),
...
write_rds(final_ir, 
          here("data", "basefiles", glue("{pub_date}_base-file_IR_underlying.rds")),
          compress = "gz")
...
write_rds(excel_data_IR, 
          here("output", glue("{pub_date}_raw_excel_data_ir1_underlying.rds")),
          compress = "gz")
...
saveWorkbook(output_ir,
             here("output", glue("{pub_date}_his_ir_qom_underlying.xlsx")),
             overwrite = TRUE)
# ### END OF SCRIPT ###