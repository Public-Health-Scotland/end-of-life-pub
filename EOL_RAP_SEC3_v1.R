##################################################################
## 01. EOE All Activity                                         ##
## Original Author : Alice Byers                                ##
## Originally written: November 2018                            ##
## Latest Update Author: Federico Centoni                       ## 
## Latest Update Date: 24/06/2019.                              ##
## Updates to script (other than standard):                     ##
...
extract_date <- "_extracted_24062019"
...
# Lookup filepaths
pc_lookup   <- "/conf/linkage/output/lookups/Unicode/Geography/Scottish Postcode Directory/Scottish_Postcode_Directory_2018_2.sav"
simd_lookup <- "/conf/linkage/output/lookups/Unicode/Deprivation/postcode_2018_2_simd2016.sav"
locality    <- "/conf/irf/05-lookups/04-geography/01-locality/Locality_lookup_source.sav"
...
names(data_01) <- tolower(names(data_01))
...
names(data_50) <- tolower(names(data_50))
...
names(data_04) <- tolower(names(data_04))
...
names(data_deaths) <- tolower(names(data_deaths))
...
#######################################
 ...
 # 1. Create SMR activity file (SMR01_50)
  ...
  smr_activity <- bind_rows(data_01,data_50) %>% 
